    ...
    record.update(
        {
            ...
            "strengths": ["确定性多维评分，覆盖来源/行为/数据/会话/协议安全五层", "支持 baseline/current 分离，满足以基准日志对比新日志的要求", "第三类 mod_sftp 协议日志纳入统一画像，可识别弱算法、协商漂移、老旧客户端指纹", "默认按账户风险聚合输出，更贴近实际安全运营视角，同时保留会话级证据", "序列聚类提供跨用户关联发现，突破单用户基线局限", "可解释且可追溯：每条告警附带触发原因和打分明细，关联发现可追溯到序列模式", "本地 JSON 管线，内网 LLM 双模式", "噪声策略和白名单可调：可信网段、可信用户、可信客户端、算法策略均可配置", "LLM 语义化串联脚本产出的孤立告警为攻击叙事，限制幻觉"],
            "limitations": [
